// src/app/pages/vehicle/vehicle.ts
import {
...
// Removed SafeHtmlPipe import (Fixing NG8113 Warning)
...
@Component({
  ...
  standalone: true,
  // Removed SafeHtmlPipe from imports
...
export class VehicleComponent implements OnInit {
  ...
  private carRentalService = inject(CarRentalService); // Injected for role check
  private router = inject(Router); // Injected for redirection

  // Expose isAdmin to the template for conditional rendering
  isAdmin = this.carRentalService.isUserAdmin();

  ngOnInit(): void {
    // SECURITY GUARD: Redirect non-admins away from management page
    if (!this.isAdmin) {
      // Allow them to stay and view the list, but hide the form and actions
      this.getAllCars();
      return;
    }
    this.getAllCars();
  ...
  onFileChange(event: Event): void {
    if (!this.isAdmin) {
      this.showNotification('error', 'Operation denied. Admin role required.');
      return;
    }
  ...
  getAllCars(): void {
    ...
    // This call is open to everyone, as customers need to see available cars.
  ...
  onSaveCar(vehicleForm: NgForm): void {
    ...
    this.isLoading.set(true);
    // Backend will block this if the user is not an admin
    this.http
  ...
  onDeleteCarById(carId: number | string): void {
    if (!this.isAdmin) {
      this.showNotification('error', 'Operation denied. Admin role required.');
      return;
    }
  ...
  onEdit(car: CarModel): void {
    if (!this.isAdmin) return; // Prevent non-admin interaction
    this.newCar.set({ ...car });
